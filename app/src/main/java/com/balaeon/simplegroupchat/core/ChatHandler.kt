...
import com.balaeon.simplegroupchat.fragments.single_chat.toDataClass
...
import com.balaeon.simplegroupchat.utils.getUnreadCount
...
class ChatHandler @Inject constructor(
    ...
    fun initHandler() {
        ...
        listenerDoc1 = messageCollectionGroup.whereArrayContains("chatUsers", fromUser!!)
            .addSnapshotListener { snapShots, error ->
                LogMessage.v("Error chatUsers error $error")
                LogMessage.v("Error chatUsers snapShots $snapShots")
                LogMessage.v("Error chatUsers metadata ${snapShots?.metadata?.isFromCache}")
    ...
    private fun onFetchDocuments() {
        messageCollectionGroup.whereArrayContains("chatUsers", fromUser!!).get().addOnSuccessListener {
            LogMessage.v("onFetchDocuments chatUsers $it")
             isFirstQuery=true
    ...
    private fun insertMessageOnDb(listOfIds: ArrayList<String>) {
        Log.d("insertMessageOnDb","$listOfIds")
        CoroutineScope(Dispatchers.IO).launch {
            val contacts = ArrayList<ChatUser>()
            val newContactIds = ArrayList<String>()  //message from new user not saved in localdb yet
